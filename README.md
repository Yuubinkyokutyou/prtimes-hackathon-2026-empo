...
初回起動時に、分析テーブルを開発環境へ再現するためのルート `init.sql` がローカルDBだけに自動実行されます。その後、`migrations/*.sql` にあるアプリ用マイグレーションが適用されます。DB のデータを作り直す場合は、開発データが消えることを確認してから `docker compose down -v` を実行し、再度起動してください。
